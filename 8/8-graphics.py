...
time_zaradka = data_time[list(data).index(max(data))]
time_razradka = max(data_time) - time_zaradka
ax.text(8, 2.6, f'Время зарядки - {time_zaradka} с', fontsize=12)
ax.text(8, 2.4, f'Время разрядки - {time_razradka} с', fontsize=12)
